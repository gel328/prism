...
每个 OAuth 源（GitHub、Google、Microsoft、Discord、Telegram、Generic OIDC、Generic OAuth 2）都已迁入 `oauth_sources` 表 — 在 **Admin → OAuth Sources** 中管理，不在本页设置。下方的旧字段仍保留以兼容历史数据，新部署应直接使用 OAuth Sources。
